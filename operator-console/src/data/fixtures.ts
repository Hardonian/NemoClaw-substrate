...
export function makePolicyPromotionProposal(overrides?: Partial<PolicyPromotionProposal>): PolicyPromotionProposal {
  return {
    proposalId: "proposal-001",
    candidate: makePolicyCandidate(),
    status: "review_required",
    notes: "Proposal-only artifact. No automatic policy mutation.",
    ...overrides,
  };
}
...
export const samplePolicyCandidates: PolicyCandidate[] = [
  ...
  makePolicyCandidate({ key: "operator_override:none:sandbox-default", reason: "repeated_override", count: 2, eventIds: ["op-006", "op-014"] }),
];

export const samplePolicyProposals: PolicyPromotionProposal[] = samplePolicyCandidates.map((c, i) =>
  makePolicyPromotionProposal({ proposalId: `proposal-${i}`, candidate: c })
);
