  ...
  if (options.length > 1) {
    selectionLoop: while (true) {
      ...
      if (isNonInteractive()) {
        ...
        // Group providers by category for easier navigation
        const cloudProviders = options.filter((o) =>
          ["build", "openai", "anthropic", "gemini", "custom", "anthropicCompatible"].includes(o.key),
        );
        const localProviders = options.filter((o) =>
          ["ollama", "install-ollama", "vllm", "install-vllm", "start-windows-ollama"].includes(o.key),
        );
        const experimentalProviders = options.filter((o) =>
          ["nim-local", "routed"].includes(o.key),
        );

        console.log("");
        console.log("  Choose your AI provider:");
        console.log("");
        console.log("  Cloud providers (no setup required):");
        let optionIndex = 0;
        const optionIndexMap = new Map<string, number>();
        for (const o of cloudProviders) {
          optionIndex++;
          optionIndexMap.set(o.key, optionIndex);
          const isDefault = o.key === "build";
          console.log(`    ${optionIndex}) ${o.label}${isDefault ? " [recommended]" : ""}`);
        }
        if (localProviders.length > 0) {
          console.log("");
          console.log("  Local providers (run models on your machine):");
          for (const o of localProviders) {
            optionIndex++;
            optionIndexMap.set(o.key, optionIndex);
            console.log(`    ${optionIndex}) ${o.label}`);
          }
        }
        if (experimentalProviders.length > 0) {
          console.log("");
          console.log("  Experimental providers:");
          for (const o of experimentalProviders) {
            optionIndex++;
            optionIndexMap.set(o.key, optionIndex);
            console.log(`    ${optionIndex}) ${o.label}`);
          }
        ...
        const envProviderIdx = envProviderHint
          ...
          : -1;
        const buildIdx = optionIndexMap.get("build") ?? 1;
        const defaultIdx =
          envProviderIdx >= 0
            ? (optionIndexMap.get(options[envProviderIdx].key) ?? buildIdx)
            : buildIdx;
        const choice = await prompt(`  Choose [${defaultIdx}]: `);
        const choiceNum = parseInt(choice || String(defaultIdx), 10);
        const selectedKey = [...optionIndexMap.entries()].find(([, idx]) => idx === choiceNum)?.[0];
