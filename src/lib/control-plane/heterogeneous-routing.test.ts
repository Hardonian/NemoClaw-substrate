...
describe("heterogeneous routing", () => {
  ...
  it("preserves local default when disabled", async () => {
    const out = await routeHeterogeneous({ requestId: "r1", nowIso: "2026-05-09T00:00:00.000Z", provider: "openai-api", model: "gpt-5.4", registry: registryWithRemote(), policyBundle: allowPolicy, governedEnabled: true, allowDegradedState: false, routingConfig: { enabled: false, source: "default" }, remoteConfig: { enabled: true, source: "env" } });
  ...
  it("ignores remote when remote execution disabled", async () => {
    const out = await routeHeterogeneous({ requestId: "r2", nowIso: "2026-05-09T00:00:00.000Z", provider: "openai-api", model: "gpt-5.4", registry: registryWithRemote(), policyBundle: allowPolicy, governedEnabled: true, allowDegradedState: false, routingConfig: { enabled: true, source: "env" }, remoteConfig: { enabled: false, source: "default" } });
  ...
  it("blocks policy denied and approval required remote", async () => {
    const denied = await routeHeterogeneous({ requestId: "r3", nowIso: "2026-05-09T00:00:00.000Z", provider: "openai-api", model: "gpt-5.4", registry: registryWithRemote(), policyBundle: denyPolicy, governedEnabled: true, allowDegradedState: false, routingConfig: { enabled: true, source: "env" }, remoteConfig: { enabled: true, source: "env" } });
    expect(denied.excludedCandidates.find((c) => c.kind === "remote_worker")?.policyEligibility).toBe("deny");

    const approval = await routeHeterogeneous({ requestId: "r4", nowIso: "2026-05-09T00:00:00.000Z", provider: "openai-api", model: "gpt-5.4", registry: registryWithRemote(), policyBundle: approvalPolicy, governedEnabled: true, allowDegradedState: false, routingConfig: { enabled: true, source: "env" }, remoteConfig: { enabled: true, source: "env" } });
  ...
  it("diagnostics include selected/excluded", async () => {
    const out = await routeHeterogeneous({ requestId: "r6", nowIso: "2026-05-09T00:00:00.000Z", provider: "openai-api", model: "gpt-5.4", registry: registryWithRemote("stale"), policyBundle: allowPolicy, governedEnabled: true, allowDegradedState: false, routingConfig: { enabled: true, source: "env" }, remoteConfig: { enabled: true, source: "env" } });
