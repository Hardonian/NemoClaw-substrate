...
describe("local runtime probes", () => {
  ...
  it("keeps diagnostics explicit and routing default unchanged", async () => {
    ...
    const lines = summarizeLocalDiagnostics({ probeSummary: out, registry: createDeviceRegistry(), governedRouting: { enabled: false, source: "default", allowDegradedStateTrigger: false } });
