...
export async function runLocalRuntimeProbes(input: LocalProbeRequest, operationalMemory?: OperationalMemoryLog): Promise<LocalProbeSummary> {
  ...
  events.unshift({ eventId: `telemetry-probe-${input.requestId}`, occurredAt: input.nowIso, sequence: -1, category: "runtime_action", source: "local-runtime-probes", provenance: { requestId: input.requestId }, replayRef: receipt.provenance, payload: { phase: "telemetry_probe_started", source: "local", confidence: telemetryAvailable ? "observed" : "unavailable" } });
