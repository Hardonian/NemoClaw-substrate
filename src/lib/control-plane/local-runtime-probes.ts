...
export async function runLocalRuntimeProbes(input: LocalProbeRequest, operationalMemory?: OperationalMemoryLog): Promise<LocalProbeSummary> {
  ...
  const receipt: ExecutionReceipt = { version: "1", receiptId: `local-probe-${input.requestId}`, requestId: input.requestId, createdAt: input.nowIso, phases: [{ phase: "received", at: input.nowIso, notes: "telemetry_probe_started" }, { phase: "completed", at: input.nowIso, notes: "telemetry_probe_finished" }], degradedEvents: degradedStates, degradedStateTriggers: [], toolInvocations: [], timing: { totalMs: 0 }, provenance: { source: "local-runtime-probes", lineage: ["diagnostics", "local-probe"], replayVersion: "1" }, operatorOverrides: [] };
