...
describe("governed provider routing", () => {
  ...
  it("disabled mode preserves default route", () => {
    const out = routeProviderWithGovernance({ requestId: "r1", nowIso: "2026-05-09T00:00:00.000Z", provider: "openai-api", model: "gpt-5.4", registry: seededRegistry(), policyBundle: allowPolicy, config: { enabled: false, source: "default", allowDegradedStateTrigger: false } });
  ...
  it("enabled mode invokes scheduler path and emits receipt/event", () => {
    ...
    const out = routeProviderWithGovernance({ requestId: "r2", nowIso: "2026-05-09T00:00:00.000Z", provider: "nvidia-nim", model: "nvidia/nemotron-3-super-120b-a12b", registry: seededRegistry(), policyBundle: allowPolicy, config: { enabled: true, source: "env", allowDegradedStateTrigger: false }, operationalMemory: memory });
  ...
  it("policy deny and approval block provider selection", () => {
    expect(() => routeProviderWithGovernance({ requestId: "r3", nowIso: "2026-05-09T00:00:00.000Z", provider: "openai-api", model: "gpt-5.4", registry: seededRegistry(), policyBundle: denyPolicy, config: { enabled: true, source: "env", allowDegradedStateTrigger: true } })).toThrow(/denied/);
    expect(() => routeProviderWithGovernance({ requestId: "r4", nowIso: "2026-05-09T00:00:00.000Z", provider: "openai-api", model: "gpt-5.4", registry: seededRegistry(), policyBundle: approvalPolicy, config: { enabled: true, source: "env", allowDegradedStateTrigger: true } })).toThrow(/approval_required/);
  });

  it("no candidate is explicit and degraded state trigger never bypasses deny", () => {
    const empty = createDeviceRegistry();
    expect(() => routeProviderWithGovernance({ requestId: "r5", nowIso: "2026-05-09T00:00:00.000Z", provider: "openai-api", model: "gpt-5.4", registry: empty, policyBundle: allowPolicy, config: { enabled: true, source: "env", allowDegradedStateTrigger: false } })).toThrow(/no eligible candidate/);
    const denied = () => routeProviderWithGovernance({ requestId: "r6", nowIso: "2026-05-09T00:00:00.000Z", provider: "openai-api", model: "gpt-5.4", registry: empty, policyBundle: denyPolicy, config: { enabled: true, source: "env", allowDegradedStateTrigger: true } });
    expect(denied).toThrow(/denied/);
  });

  it("diagnostics shows enabled/disabled state", () => {
    expect(summarizeGovernedRoutingDiagnostics({ enabled: false, source: "default", allowDegradedStateTrigger: false })[0]).toContain("disabled");
    expect(summarizeGovernedRoutingDiagnostics({ enabled: true, source: "env", allowDegradedStateTrigger: true }, { provider: "nvidia-nim", model: "m", receiptId: "rid" })[0]).toContain("enabled");
  });
});
