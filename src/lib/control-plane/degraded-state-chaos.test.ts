...
function fakeProbe(status: WorkerProbeResult["status"], overrides: Partial<WorkerProbeResult["telemetry"]> = {}): WorkerProbeResult {
  return {
    ...
    degradedStates: status === "degraded" ? [{ category: "degraded", reason: "probe degraded", affectedSubsystem: "probe", severity: "warning", reasonCode: "unknown_error" as const, explanation: "probe degraded", sourceComponent: "worker-probes", timestamp: "2026-05-09T00:00:00.000Z" }] : [],
  };
}
