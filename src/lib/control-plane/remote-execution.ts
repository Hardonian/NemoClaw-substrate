...
import { type OperationalEvent, type OperationalMemoryLog, buildEventsFromReceipt, FileOperationalMemoryStore } from "./operational-memory";
import {
...
export interface RemoteExecutionRequest { requestId: string; nowIso: string; action: string; command: string; commandDescriptor?: CommandDescriptor; nodeId?: string; targetEndpoint?: string; auth?: { headerName?: string; token?: string }; approved?: boolean; timeoutMs?: number; executionPlanRequired?: boolean; commandPolicy?: CommandExecutionPolicy; transportType?: TransportType; sshCredentials?: SshCredentials; expectedOutputHash?: string; }
export interface RemoteExecutionResult { status: RemoteExecutionStatus; output?: string; degradedReason?: string; errorCode?: string; receipt: ExecutionReceipt; events: OperationalEvent[]; replayRef: ExecutionReceipt["provenance"]; outputHash?: string; }
...
export interface RemoteExecutionReceiptContext { transport: TransportType; target: string; policyDecision: "allow" | "deny" | "approval_required" | "disabled"; redactedAuth: Record<string, string>; }
...
export interface RemoteWorkerProofConfig {
  ...
  credentials: SshCredentials | { endpoint: string; signingKey: string };
  timeoutMs: number;
...
export async function runRemoteExecution(input: { request: RemoteExecutionRequest; config: RemoteExecutionConfig; transport: RemoteExecutionTransport; policyBundle: PolicyBundle; registry: DeviceRegistry; operationalMemory?: OperationalMemoryLog; executionPlan?: ExecutionPlan; executionApproval?: ExecutionApproval }): Promise<RemoteExecutionResult> {
  ...
  const transportType: TransportType = request.transportType ?? "http";
  ...
  const receiptBase = { version: "1" as const, receiptId: `remote-exec-${request.requestId}`, requestId: request.requestId, createdAt: request.nowIso, phases: [{ phase: "received", at: request.nowIso, notes: "execution_requested" }], toolInvocations: [], timing: { totalMs: 0 }, degradedStateTriggers: [], operatorOverrides: [], provenance: { source: "remote-execution", lineage: ["worker", "remote"], replayVersion: "1" as const } };
  const context: RemoteExecutionReceiptContext = { transport: transportType, target, policyDecision, redactedAuth: redact(request.auth) };
  const finalize = (status: RemoteExecutionStatus, notes: string, degradedReason?: string, errorCode?: string, plan?: ExecutionPlan, outputHash?: string): RemoteExecutionResult => {
    ...
    return { status, degradedReason, errorCode, receipt, events, replayRef: receipt.provenance, outputHash };
  };
  ...
  if (input.executionPlan) {
    const currentPolicySnapshot = createExecutionPolicySnapshot({
      ...
      policy: policyEval,
      degradedStateTriggerPermitted: input.executionPlan.policySnapshot.degradedStateTriggerPermitted,
  ...
  if (transportType === "ssh") {
    if (!request.sshCredentials) return finalize("failed", "missing_ssh_credentials", "missing_ssh_credentials", "constraint_unsatisfied");
    ...
    try {
      const proofResult = await executeRemoteWorkerProof({
        transportType: "ssh",
        credentials: request.sshCredentials,
        timeoutMs: commandDecision.descriptor.timeoutMs,
        expectedOutputHash: request.expectedOutputHash,
      }, safeCommand);
      if (!proofResult.success) {
        return finalize("degraded", proofResult.error ?? "ssh_execution_failed", proofResult.error ?? "ssh_execution_failed", "transport_unreachable", input.executionPlan, proofResult.outputHash);
      }
      if (!proofResult.hashMatches) {
        return finalize("failed", "output_hash_mismatch", "output_hash_mismatch", "constraint_unsatisfied", input.executionPlan, proofResult.outputHash);
      }
      return { ...finalize("succeeded", "execution_succeeded", undefined, undefined, input.executionPlan, proofResult.outputHash), output: proofResult.output };
    } catch (e) {
      const msg = e instanceof Error ? e.message.toLowerCase() : String(e).toLowerCase();
      return finalize("degraded", msg.includes("timeout") ? "timeout" : "network_unavailable", msg.includes("timeout") ? "timeout" : "network_unavailable", "transport_unreachable", input.executionPlan);
    }
  }

  if (transportType === "https-signed") {
    const endpoint = request.targetEndpoint;
    ...
    const signingKey = typeof request.auth?.token === "string" ? request.auth.token : "";
    if (!signingKey) return finalize("failed", "missing_signing_key", "missing_signing_key", "constraint_unsatisfied");
    try {
      const proofResult = await executeRemoteWorkerProof({
        transportType: "https-signed",
        credentials: { endpoint: urlDecision.url.toString(), signingKey },
        timeoutMs: commandDecision.descriptor.timeoutMs,
        expectedOutputHash: request.expectedOutputHash,
      }, commandDecision.descriptor.name);
      if (!proofResult.success) {
        return finalize("degraded", proofResult.error ?? "https_signed_failed", proofResult.error ?? "https_signed_failed", "transport_unreachable", input.executionPlan, proofResult.outputHash);
      }
      if (!proofResult.hashMatches) {
        return finalize("failed", "output_hash_mismatch", "output_hash_mismatch", "constraint_unsatisfied", input.executionPlan, proofResult.outputHash);
      }
  ...
  try {
    ...
    const outputHash = body.output ? computeOutputHash(body.output) : undefined;
    if (request.expectedOutputHash && outputHash && !verifyResultHash(outputStr, request.expectedOutputHash)) {
      return finalize("failed", "output_hash_mismatch", "output_hash_mismatch", "constraint_unsatisfied", input.executionPlan, outputHash);
    }
    const ok = finalize("succeeded", "execution_succeeded", undefined, undefined, input.executionPlan, outputHash);
