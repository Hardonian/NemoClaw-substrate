...
import { readJsonSyncSafe } from "../core/json-file";
...
export function rejectDuplicateExecution(runDir: string, key: string): void {
  ...
  const records = readJsonSyncSafe<IdempotencyRecord[]>(file, []);
...
export function enqueueExecution(runDir: string, item: ExecutionQueueItem): ExecutionQueueItem {
  ...
  const queue = readJsonSyncSafe<ExecutionQueueItem[]>(queueFile, []);
  queue.push(item);
  write(queueFile, queue.sort((a,b)=>a.id.localeCompare(b.id)));
  const id = readJsonSyncSafe<IdempotencyRecord[]>(idFile, []);
...
export function acquireLease(runDir: string, lease: ExecutionLease): ExecutionLease {
  ...
  const leases = readJsonSyncSafe<ExecutionLease[]>(file, []);
...
export function releaseLease(runDir: string, leaseId: string): void { const file=path.join(runDir,"lease.json"); const leases=readJsonSyncSafe<ExecutionLease[]>(file,[]).map((l)=>l.id===leaseId?{...l,status:"released" as const}:l); write(file,leases); }
export function expireLease(runDir: string, leaseId: string): void { const file=path.join(runDir,"lease.json"); const leases=readJsonSyncSafe<ExecutionLease[]>(file,[]).map((l)=>l.id===leaseId?{...l,status:"expired" as const}:l); write(file,leases); }

export function validateOwnership(runDir: string, ownership: ExecutionOwnership): void {
  const leases = readJsonSyncSafe<ExecutionLease[]>(path.join(runDir,"lease.json"),[]);
...
export function readQueueState(runDir: string): ExecutionQueueItem[] { return readJsonSyncSafe<ExecutionQueueItem[]>(path.join(runDir,"queue.json"),[]); }
export function readLeaseState(runDir: string): ExecutionLease[] { return readJsonSyncSafe<ExecutionLease[]>(path.join(runDir,"lease.json"),[]); }
