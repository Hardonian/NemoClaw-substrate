...
export function backupSandboxState(sandboxName: string, options: BackupOptions = {}): BackupResult {
  ...
  try {
    if (stateDirs.length > 0) {
      const bsdResult = backupStateDirectories(configFile, sandboxName, dir, stateDirs, backupPath);
      backedUpDirs.push(...bsdResult.backedUpDirs);
      failedDirs.push(...bsdResult.failedDirs);

      if (bsdResult.earlyReturn) {
        return {
          success: false,
          manifest,
          backedUpDirs,
          ...
          backedUpFiles,
          failedFiles: stateFiles.map((f) => f.path),
          ...(bsdResult.error ? { error: bsdResult.error } : {})
        };
      }
    }
