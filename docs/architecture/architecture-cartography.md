...
### 0.1 Substrate Relationships

```mermaid
graph TD
    subgraph "Operator (High Trust)"
        CLI[Operator CLI]
        Console[Operator Console]
    end

    subgraph "Control Plane (Governed)"
        Router[Deterministic Router]
        Policy[Policy Engine]
        Evidence[Evidence Collector]
    end

    subgraph "Execution Plane (Untrusted)"
...
sequenceDiagram
    ...
    O->>CP: Request Execution
    CP->>CP: Evaluate Policy
    alt Policy PASS
        CP->>EP: Dispatch to Sandbox
        EP->>EP: Execute Task
        EP->>OB: Emit Raw Telemetry
        OB->>OB: Redact Secrets
        OB->>CP: Return Governed Receipt
        CP->>O: Execution Receipt (Success)
    else Policy FAIL
        CP->>CP: Generate Denial Record
        CP->>O: Rejection (Fail-Closed)
    end
```
