<!-- SPDX-FileCopyrightText: Copyright (c) 2026 NVIDIA CORPORATION & AFFILIATES. All rights reserved. -->
<!-- SPDX-License-Identifier: Apache-2.0 -->

# Architecture Cross-Reference Index

This index provides a thematic cross-reference for discovering architecture documents based on functional domains.

## Governance & Coherence

- [Governance Invariants](governance-invariants.md)
- [Governance Glossary](governance-glossary.md)
- [Canonical Terminology Index](canonical-terminology-index.md)
- [Architecture Cartography](architecture-cartography.md)
- [Governance Map](governance-map.md)
- [Subsystem Ownership Map](subsystem-ownership-map.md)
- [Status Matrix](capability-status-matrix.md)

## Security & Threat Modeling

- [Security Threat Model](security-threat-model.md)
- [Governance Threat Model](governance-threat-model.md)
- [Security Boundaries](security-boundaries.md)
- [Secret Redaction Doctrine](secret-redaction-doctrine.md)

## Execution & Routing

- [Execution Lifecycle Substrate](execution-lifecycle-substrate.md)
- [Heterogeneous Routing](heterogeneous-routing.md)
- [Remote Execution Adapters](remote-execution-adapters.md)

## Observability & Verification

- [Receipts and Degraded States](receipts-and-degraded-states.md)
- [Evidence Topology](evidence-topology.md)
- [Replayability](replayability.md)
