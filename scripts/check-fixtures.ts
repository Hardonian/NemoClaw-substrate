...
function main(): void {
  ...
  } = require("../src/lib/control-plane/fixture-generators");
  ...
  const fixtures = [
    ...
    },
    {
      ...
      data: { items: queueLeaseResult.items, leases: queueLeaseResult.leases },
    },
    {
    ...
    },
    {
      ...
      data: {
        events: seededResult.events.filter(
          (e: { category: string }) =>
            e.category === "diagnostics_snapshot" || e.category === "replay_metadata",
        ),
    ...
    },
    {
      name: "receipt.json",
      data: {
        events: seededResult.events.filter((e: { category: string }) => e.category === "receipt"),
        manifestHash: seededResult.manifestHash,
      },
    },
    {
      ...
      data: {
        ...
        version: "1.0.0",
        events: replayResult.envelope.events.map(
          (e: { category: string; eventId: string; occurredAt: string }, i: number) => ({
            type:
              i === 0 ? "start" : i === replayResult.envelope.events.length - 1 ? "end" : "event",
            category: e.category,
            eventId: e.eventId,
  ...
  if (committedFiles.length !== generatedFiles.length) {
    console.error(
      `FAILED: Committed fixtures (${committedFiles.length}) != generated fixtures (${generatedFiles.length})`,
    );
    process.exit(1);
  }
  ...
  console.log(
    `OK: All ${committedFiles.length} fixtures match generator output (seed=${PINNED_SEED}).`,
  );
}
