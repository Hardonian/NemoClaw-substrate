...
function resolveBundlePath(bundleName: string, bundleVersion: string, outputDir: string): string {
...
function expectedBundleFiles(): string[] {
  return ["bin/start.sh", "config/nemoclaw.conf", "health/check.sh", "VERSION", "CHECKSUMS"];
}
...
describe("package-bundle logic", () => {
  describe("resolveBundlePath", () => {
    it("should construct path from name, version, and output dir", () => {
      const result = resolveBundlePath("nemoclaw-bundle", "20260101", "./bundles");
